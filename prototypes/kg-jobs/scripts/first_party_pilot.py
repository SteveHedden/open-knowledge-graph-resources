...
PILOT = Namespace("https://openknowledgegraphs.com/prototypes/kg-jobs/first-party-pilot/")
...
def _organization_index(path: Path = ROOT / "data" / "organizations.json") -> tuple[dict, dict]:
    ...
    *, live: bool = False, fixtures: Path | None = None,
    ...
    for key in requested:
        ...
        try:
            ...
            classified = classify_records(normalized, match_terms)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        runtime_dir = RUNTIME_DIR if args.live else FIXTURE_RUNTIME_DIR
        run = run_pilot(
